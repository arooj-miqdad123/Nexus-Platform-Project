...
import { getMyProfile } from '../../api'; // Backend API endpoint hook

export const InvestorProfile: React.FC = () => {
    const { id } = useParams<{ id: string }>();
    const { user: currentUser } = useAuth();

    // Real dynamic states for investor database matching
    ...
    useEffect(() => {
        const loadProfileData = async () => {
            try {
                setLoading(true);
                // Strict Type-Safe Cross Checking (Number vs String)
                const isSelf = currentUser?.id === id || String(currentUser?.id) === id;

                if (isSelf) {
                    // Dynamic data from core system backend profile database
                    const response = await getMyProfile();
                    setInvestor(response as unknown as Investor);
                } else {
                    // Fallback parsing strategy for other profiles view
                    let localUser = findUserById(id || '') as Investor | null;
                    if (!localUser && id) {
                        localUser = findUserById(String(id)) as Investor | null;
                    }
                    setInvestor(localUser);
                }
            } catch (error) {
                console.error("Error matching or downloading profile structure:", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return <div className="text-center py-12 text-gray-600 font-medium animate-pulse">Loading Investor Portfolio Profile...</div>;
    }
    ...
    const isCurrentUser = currentUser?.id === investor.id || String(currentUser?.id) === String(investor.id);
