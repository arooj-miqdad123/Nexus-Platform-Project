...
export const Navbar: React.FC = () => {
    ...
    // ✅ Lowercase role handle kiya taaki App.tsx ke route match hon
    const userRoleLower = user?.role?.toLowerCase();

    // User dashboard route based on role
    const dashboardRoute = userRoleLower === 'entrepreneur'
        ? '/dashboard/entrepreneur'
        : '/dashboard/investor';

    // ✅ FIX: Route paths match App.tsx structure perfectly now
    const profileRoute = user
        ? `/profile/${userRoleLower}/${user.id}`
        : '/login';

    const navLinks = [
        {
            icon: userRoleLower === 'entrepreneur' ? <Building2 size={18} /> : <CircleDollarSign size={18} />,
            text: 'Dashboard',
            path: dashboardRoute,
        },
        {
